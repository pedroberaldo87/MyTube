...
function ChannelCard({ channel, folders, tags, expanded, onToggleExpand, onRefresh }: ChannelCardProps) {
  const [saving, setSaving] = useState(false)
  const [selectedFolder, setSelectedFolder] = useState(channel.folderId ?? '')
  const [selectedTagIds, setSelectedTagIds] = useState<string[]>(channel.tagIds)
  const [confirmDelete, setConfirmDelete] = useState(false)

  const assignedFolder = folders.find(f => f.id === channel.folderId)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('[Dashboard] Failed to update channel:', err)
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      await sendMessage({ type: 'DELETE_ORGANIZABLE', payload: { id: channel.id } })
      await onRefresh()
    } catch (err) {
        ...
        <div style={cardStyles.cardInfo}>
          ...
          <div style={cardStyles.metaRow}>
            ...
            {!channel.isSubscribed && (
              <span style={cardStyles.manualBadge}>Manual</span>
            )}
          </div>
          <div style={cardStyles.tagsRow}>
            {channel.tagIds.map(id => {
        ...
        <div style={cardStyles.editPanel}>
          ...
          <div style={cardStyles.editActions}>
            ...
            <button
              ...
              onClick={() => {
                setSelectedFolder(channel.folderId ?? '')
                setSelectedTagIds(channel.tagIds)
                onToggleExpand()
              }}
            >
              Cancel
...
export function Channels({ appState, onRefresh }: ChannelsProps) {
  const { organizables, folders, tags } = appState
  const channels = organizables.filter(o => o.type === 'channel')

  const [expandedId, setExpandedId] = useState<string | null>(null)
  const [filterFolder, setFilterFolder] = useState('')
  const [filterTags, setFilterTags] = useState<string[]>([])
  const [filterSubscribed, setFilterSubscribed] = useState<'all' | 'subscribed' | 'manual'>('all')
  const [syncMessage, setSyncMessage] = useState<string | null>(null)

  const handleSync = () => {
    setSyncMessage('Navigate to youtube.com/feed/channels, wait for it to load, then click Sync Subscriptions again.')
  }
  ...
  const filtered = channels.filter(ch => {
    if (filterFolder !== '' && ch.folderId !== filterFolder) return false
    if (filterTags.length > 0 && !filterTags.every(t => ch.tagIds.includes(t))) return false
    if (filterSubscribed === 'subscribed' && !ch.isSubscribed) return false
    if (filterSubscribed === 'manual' && ch.isSubscribed) return false
    return true
  })
    ...
    <div>
      <div style={pageStyles.pageHeader}>
        <h2 style={pageStyles.pageTitle}>
          Channels <span style={{ color: 'var(--mt-text-secondary)', fontSize: 16 }}>({channels.length})</span>
        </h2>
        <button style={pageStyles.syncBtn} onClick={handleSync}>Sync Subscriptions</button>
      </div>
      ...
      <div style={pageStyles.filters}>
        ...
        <select
          ...
          onChange={e => setFilterSubscribed((e.target as HTMLSelectElement).value as 'all' | 'subscribed' | 'manual')}
        >
      ...
      {filtered.length === 0 && (
        <p style={{ color: 'var(--mt-text-secondary)', textAlign: 'center', marginTop: 40 }}>
          {channels.length === 0
            ? 'No channels yet. Sync your subscriptions to get started.'
